...
export default function SprintSummary({
  ...
  const statusBadge = {
    upcoming: { bg: 'bg-yellow-100 dark:bg-yellow-900', text: 'text-yellow-800 dark:text-yellow-200', label: 'Upcoming' },
    active: { bg: 'bg-green-100 dark:bg-green-900', text: 'text-green-800 dark:text-green-200', label: 'Active' },
    completed: { bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-800 dark:text-gray-200', label: 'Completed' },
  }[sprintStatus];

  return (
    <div
      onClick={onSelect}
      className={`bg-white rounded-lg shadow p-4 cursor-pointer transition-all hover:shadow-md dark:bg-gray-800 dark:border dark:border-gray-700 dark:hover:border-gray-600 ${
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-sm">
        <div>
          <div className="font-semibold text-gray-800 dark:text-gray-100">{issues.length}</div>
          <div className="text-gray-500 dark:text-gray-400">Issues</div>
        </div>
        <div>
          <div className="font-semibold text-gray-800 dark:text-gray-100">{daysElapsed}</div>
          <div className="text-gray-500 dark:text-gray-400">Days In</div>
        </div>
        <div>
          <div className="font-semibold text-gray-800 dark:text-gray-100">{daysRemaining}</div>
