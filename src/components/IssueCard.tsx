      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 truncate">{issue.title}</h3>
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className={`badge-${issue.priority.toLowerCase()}`}>
                {priorityLabels[issue.priority as IssuePriority]}
              </span>
              {issue.storyPoints && (
                <span className="px-2 py-0.5 bg-purple-100 text-purple-800 rounded-full text-xs font-medium dark:bg-purple-900/40 dark:text-purple-300">
